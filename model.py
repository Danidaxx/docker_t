import pickle
import time,re,csv,sys,uuid,joblib
...
def _model_train(df,tag,test=False):
    ...
    pipe_rf.fit(X, y)
    print ("Random forest")
    print("Score:", pipe_rf.score(X, y))
    
    yhat = pipe_rf.predict(X)
    ...
    y_pred = pipe_gbt.predict(X_test)
    eval_rmse =  round(np.sqrt(mean_squared_error(y_test,y_pred)))
    
    ## retrain using all data
    pipe_gbt.fit(X, y)
    print ("Gradient boosting tree")
    print("Score:", pipe_gbt.score(X, y))
    
    yhat = pipe_gbt.predict(X)
    SS_Residual = sum((y-yhat)**2)
    SS_Total = sum((y-np.mean(y))**2)
    r_squared = 1 - (float(SS_Residual))/SS_Total
    adj_r_squared = 1 - (1-r_squared)*(len(y)-1)/(len(y)-X.shape[1]-1)
    ...
    model_name = re.sub("\.","_",str(MODEL_VERSION+1))
    if test:
        saved_model = os.path.join(MODEL_DIR,
                                   "test-{}-{}.joblib".format(tag,model_name))
        print("... saving test version of model: {}".format(saved_model))
    ...
    joblib.dump(pipe_gbt,saved_model)
    ...
    update_train_log(tag,(str(dates[0]),str(dates[-1])),{'rmse':eval_rmse},runtime,
                     MODEL_VERSION+1, MODEL_VERSION_NOTE,test=test)
