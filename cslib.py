...
def fetch_ts(data_dir, clean=False):
    ...
    file_list = [os.path.join(data_dir,f) for f in os.listdir(data_dir) if re.search("\.json",f)]
    countries = [os.path.join(data_dir,"ts_"+re.sub("\s+","_",c.lower()) + ".csv") for c in top_ten_countries]
    ...
    for country in top_ten_countries:
        ...
        file_name = os.path.join(data_dir,"ts_"+ country_id + ".csv")
        dfs[country_id] = convert_to_ts(df,country=country)

    ## save the data as csvs    
    for key, item in dfs.items():
        item.to_csv(os.path.join(ts_data_dir,"ts_"+key+".csv"),index=False)
...
def data_ingestion():
    ...
    print ('PRINT TOP 10 COUNTRIES')
    df_top10 = df.loc[df['country'].isin(['United Kingdom', 'EIRE', 'Germany', 'France', 'Norway', 'Spain', 'Hong Kong', 'Portugal', 'Singapore', 'Netherlands'])]
    print (df_top10)
    
    print ('ROWS WITH NULL VALUES')
    print(df.isnull().sum())
    
    s = df['invoice_date'].max()-df['invoice_date'].min()
    print ('RANGE BETWEEN FIRST AND LAST INVOICE DATE')
    print(s)
    print ('NUMBER OF UNIQUE INVOICE DATES')
    print(df['invoice_date'].nunique())

    
    print ('REVENES BY COUNTRY')
    states = df.groupby(['country'] )['price'].agg('sum').sort_values(ascending=False).head(10)
    states.plot(kind='bar')
    plt.show()

    
    
    print ('REVENUES IN SPECIFIC YEARS')
    ...
    print ('REVENUES IN SPECIFIC MONTHS')
    ss = df.groupby(['month'] )['price'].agg('sum').sort_values(ascending=False).head(10)
    ss.plot(kind='bar', color="black")
    plt.show()
